...
### 3.3 PaymentNotifyLog（回调留痕）—— 非聚合根
微信支付/分账回调原始报文落库，供对账与排查。
...
app(PaymentCallbackAppService):
  1. WechatPayClient.verifyAndParse 验签+解密 → 落 payment_notify_log
...
SettlementAppService.share(paymentOrder):
  ...
  4. 微信分账无实时结果 → SettlementReconcileJob 定时 queryProfitShare → FINISHED/FAILED
```
...
- **流水**：`payment_order`（收款）+ `payment_settlement`（分账）+ `payment_notify_log`（回调原文）三层留痕。
...
- **回调漏处理补偿**：扫 `payment_notify_log.process_status=RECEIVED` 超时未处理的重放。
...
**原则：支付状态是支付域的读模型，不沉淀到 registration（不加字段、不扩 `RegistrationStatusEnum`），也不写进约球域 DTO。** 支付状态唯一真相来源是 `payment_order.status`（回调驱动），展示时由 **app 层组合响应**承载，约球域 `MeetupDetailDTO`/`ParticipantDTO` 零改动、不 import 支付域。

理由：registration 是「报名生命周期」、payment 是正交的「支付生命周期」，混入同一枚举/字段会导致二维状态爆炸、跨域回写耦合、回调乱序的一致性风险，并阻碍后续退款/多渠道扩展。约球域 DTO 不 import 支付域，保持约球域对支付域零依赖（仅 app 层同时依赖两域做组合）。

做法（详情页）：app 层详情接口返回组合响应 `MeetupDetailPageDTO`（rally-app），持有约球域 `MeetupDetailDTO detail`（原样）与支付域 `MeetupPaymentViewDTO payment`，二者并列、互不引用。
```java
MeetupDetailDTO detail = meetupAppService.detail(meetupId, userId);   // 约球域原详情，无支付字段
...
MeetupPaymentViewDTO payment = PaymentAppConvertMapper.INSTANCE.toView(payMap, meetupId, userId);
return new MeetupDetailPageDTO(detail, payment);                       // app 层组合，前端按 userId 合并逐人支付态
```
- 关联键 `(meetup_id, payer_user_id)`，`payer_user_id == registration.user_id`；逐人支付态放 `payment.participantStatus`（userId -> 态），前端用 `detail.participants[i].userId` 查询，**不 join**、不回写 `ParticipantDTO`。
...
支付交互按角色拆成两个正交维度，统一收口到 app 层组合响应的 `payment` 字段（支付域 `MeetupPaymentViewDTO`），约球域 `MeetupDetailDTO`/`ParticipantDTO` 与 registration 零改动、零 import：
...
- 组合响应 `MeetupDetailPageDTO { MeetupDetailDTO detail; MeetupPaymentViewDTO payment; }` 定义在 app 层，`MeetupPaymentViewDTO` 与视图枚举定义在支付域；约球域不感知支付域，组合只发生在 app 层。
...
- **待验证坑**：`relation_type`（个人一般 `USER`/`PARTNER`，按微信文档定）；大额分账可能需个人真实姓名加密校验（小额免）。
...
| `payment_notify_log` | —（日志） | 回调原始报文留痕，对账/排查 |
...
rally-domain/com/rally/domain/payment/
  ├─ model/    PaymentOrder, SettlementOrder, ShareReceiver, PaymentNotifyLog, PrepayResult, ChannelTradeResult, ChannelShareResult, CallbackResult, MeetupPaymentViewDTO
  ├─ enums/    PayChannelEnum, BizTypeEnum, PaymentStatusEnum, SettlementStatusEnum, ShareReceiverStatusEnum, PaymentViewStatus, CollectionStateEnum
  ├─ gateway/  PaymentChannelClient, PaymentOrderRepository, SettlementOrderRepository, ShareReceiverRepository, PaymentNotifyLogRepository
...
rally-app/com/rally/payment/
  ...
  ├─ model/    MeetupDetailPageDTO（组合约球域 MeetupDetailDTO + 支付域 MeetupPaymentViewDTO，详情页响应；约球域不感知支付域）
  └─ convert/ PaymentAppConvertMapper（MapStruct，出参 DTO / 入参 Cmd）

注：约球域 MeetupDetailDTO/ParticipantDTO 零改动、不 import 支付域；MeetupPaymentViewDTO 与 PaymentViewStatus/CollectionStateEnum 定义在支付域，组合只在 app 层 MeetupDetailPageDTO 发生。

rally-infrastructure/com/rally/
  ├─ client/pay/wechat/  WechatPayClient(implements PaymentChannelClient), WechatPayProperties
  └─ db/payment/         PaymentOrderPO/Mapper/Service/RepositoryImpl, SettlementOrder*, ShareReceiver*, PaymentNotifyLog*
...
- **D8 分账接收方账本** ✅ 我方维护 `payment_share_receiver`，发起收款时 `ensureBound` 提前绑定，`last_share_time` 取发起收款时间；上限 LRU 淘汰列入后续（见 §5.7）。待验证：`relation_type`、个人姓名校验。
- **D9 详情页支付交互** ✅ `ActionStateEnum` 不扩展支付态（避免 评价×支付×角色 二维爆炸、守约球域零改动）；支付交互按角色拆为发起人维度 `collectionState` 与参与人维度 `myPaymentStatus` 两个正交字段，挂在详情 DTO 的 `payment` 子对象，app 层 read 组装（见 §5.6）。
...
### 14.1 架构与解耦红线（不可破坏）
- **约球域零改动**：不改 `RegistrationStatusEnum`、不在 registration 加支付字段。支付状态只在 app 层 read 编排时关联 `payment_order` 组装（§5.5）。
- **支付域不依赖约球域**：支付域只认 `meetupId + payer/payee + bizType`；取参与者名单/发起人由 app 层调约球域**只读查询**完成。
...
- 回调：`verifyAndParse` 必须**验签 + APIv3 解密**，原文落 `payment_notify_log`，并按微信要求返回成功应答格式。
- **待联调前确认的三方资质**（外部依赖，阻塞联调，非代码）：商户号、APIv3 密钥、商户证书/私钥、**分账权限开通**、发起人个人分账接收方 `relation_type`、大额是否需姓名校验。

### 14.4 项目规范对齐（编码前必读 CLAUDE.md / rules）
- 新增配置键须**同时**：(a) 在 `SystemConfigKey` 枚举登记；(b) `sys_config` 落库（已在 SQL 第 5 节给出 4 个键）。
...
### 15.4 PaymentNotifyLog（回调留痕）—— 非聚合根

```java
public class PaymentNotifyLog {
    public static PaymentNotifyLog received(PayChannelEnum channel, String bizType, String rawBody); // 落 RECEIVED
    public void markProcessed();  // RECEIVED → PROCESSED
...
| 支付回调 §5.2 | 落 `payment_notify_log` + 返回微信应答 | `PaymentDomainService.markPaid`（`PaymentOrder.markPaid` 幂等）→ `SettlementDomainService.share` |
...
| 状态透出 §5.5/§5.6 | app 组合 `MeetupDetailPageDTO`（约球详情 + `payment`），约球域不感知支付域 | `PaymentQueryDomainService.statusByMeetup` / `hasAnyOrder` / `hasPending`（`PaymentOrder.toViewStatus`） |
