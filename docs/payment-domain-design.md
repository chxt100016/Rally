...
以下为外部资质与密钥，需人工从微信商户平台/小程序后台获取后填入，是联调的前置阻塞项（非代码）。除 `fee_rate` 等 4 个已落 `sys_config`（§9）外，密钥类统一进 `application-prod.yml` 的 `wechat.pay.*`（`@ConfigurationProperties("wechat.pay")` → `WechatPayProperties`），证书/私钥文件不入库、不进 git。

| # | 项 | 配置 key / 落点 | 从哪获取 | 值（待填） |
|---|---|---|---|---|
| 1 | 商户号 | `wechat.pay.mch-id` | 微信支付商户平台 → 账户中心 | ____ |
| 2 | 小程序 AppID | `wechat.pay.app-id`（复用 `wechat.mini.app-id`） | 小程序后台 | ____ |
| 3 | APIv3 密钥 | `wechat.pay.api-v3-key` | 商户平台 → API 安全 → 设置 APIv3 密钥 | ____ |
| 4 | 商户证书序列号 | `wechat.pay.merchant-serial-number` | 商户平台 → API 安全 → 申请 API 证书后查看 | ____ |
| 5 | 商户 API 私钥 | `wechat.pay.private-key-path`（`apiclient_key.pem` 路径） | 申请 API 证书时下载 | ____ |
| 6 | 支付回调地址 | `wechat.pay.pay-notify-url` | 我方域名 + `/api/rally/wechat/pay/notify`（需公网可达、HTTPS） | ____ |
| 7 | 分账回调地址（可选） | `wechat.pay.share-notify-url` | 我方域名（用回调则填，否则靠 Job 查询对账） | ____ |
| 8 | 分账接收方 relation_type | `wechat.pay.share-receiver.relation-type`（默认 `USER`/`PARTNER`） | 微信分账文档/实测确认 | ____ |
| 9 | 分账接收方上限 | `payment.wechat.share_receiver_max`（已落 `sys_config`，默认 20000） | 微信文档 | 20000 |
| 10 | 发起人个人分账接收方 openid | 运行时由登录态 user→openid 取，无需静态填 | 登录态映射 | （动态） |

外部需开通/确认（无配置项，属商户资质动作）：
- 商户号已**开通分账权限**（未开通则 `profitShare` 直接失败）。
- 大额分账是否需**个人真实姓名加密校验**（小额一般免，按商户等级与金额实测）。
- 商户证书/APIv3 密钥在有效期内，回调域名已在小程序/商户后台**配置并通过校验**。
