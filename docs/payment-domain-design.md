...
**原则：支付状态是支付域的读模型，不沉淀到 registration（不加字段、不扩 `RegistrationStatusEnum`）。** 支付状态唯一真相来源是 `payment_order.status`（回调驱动），展示时由 **app 层 read 编排**关联组装，约球域零改动。

理由：registration 是「报名生命周期」、payment 是正交的「支付生命周期」，混入同一枚举/字段会导致二维状态爆炸、跨域回写耦合、回调乱序的一致性风险，并阻碍后续退款/多渠道扩展。

做法（详情页）：
```java
List<RegistrationData> participants = meetup.getParticipants(userId);
...
// merge：ParticipantDTO.paymentStatus = payMap.getOrDefault(uid, NONE)
```
- 关联键 `(meetup_id, payer_user_id)`，`payer_user_id == registration.user_id`，内存按 user_id 合并，**不 join**（贴合「单表查 + 内存组装」风格）。
...
支付交互按角色拆成两个正交维度，作为详情 DTO 上的独立读模型字段，由 app 层详情编排组装（约球域与 registration 零改动）：
...
| 参与人列表·逐人 | `ParticipantDTO.paymentStatus` | 发起人看列表收款进度 | `NONE/UNPAID/PAID/CLOSED` |

- 「待支付」是参与人维度（当前用户支付单态），「何时可发起收款」是发起人维度（本场是否已有单），两字段分别表达、互不污染 actionState。
- `payment.collectionState` 由 `PaymentQueryDomainService.hasAnyOrder/hasPending` 推导；`payment.myPaymentStatus` 取当前用户本场 `payment_order` 的 `toViewStatus()`；逐人 `paymentStatus` 由 `statusByMeetup` 按 `payer_user_id` 合并。
- 详情 DTO 新增 `payment` 子对象 `MeetupPaymentViewDTO { collectionState, myPaymentStatus }`，与现有 `recap`/`weather` 子对象同构；这些视图枚举定义在支付域，约球域 read DTO 引用属允许的正向依赖。
...
| 状态透出 §5.5 | app read 组装详情 DTO 支付字段 | `PaymentQueryDomainService.statusByMeetup` / `hasAnyOrder` / `hasPending`（`PaymentOrder.toViewStatus`） |
