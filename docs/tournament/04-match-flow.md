...
- `submitBooking(matchId, userId, 球场+时间)`：订场人提交赛约，BOOKING → SCHEDULED，写 scheduleSubmittedTime。
- `handleScheduleConfirm(matchId, userId, action, reasonCode?, text?)`：非订场人处理赛约。
  - 接受：置本人 confirmStatus=CONFIRMED；全员 CONFIRMED → 创建 Meetup（模块间调约球域，meetupType=TOURNAMENT、关联 matchId）→ 待比赛。
  - 打回重订：退回 BOOKING、重置所有 confirmStatus=PENDING、记 last_rebook_*，不计拒绝、无上限。
  - 拒绝比赛：REJECTED，写 reject_phase=SCHEDULE_REJECT，全员回池，拒绝方 +1 拒绝次数（按 stage 判上限）。
...
提交赛约。入参 matchId + courtName/courtAddress + scheduledStartTime + scheduledDuration（支持小数）。仅订场人、仅 BOOKING 态。不需定比赛形式。
