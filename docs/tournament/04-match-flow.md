...
- **TournamentMatch（聚合根）+ MatchParticipant（聚合内实体）**。
  ...
  - Participant 承载各人 confirmStatus / resultConfirmStatus / entryNo。
...
- `selectCourtBooker(matchId, userId)`：MATCHED 状态下抢订场人，使用乐观锁防并发。
- `rejectOnAwaitCourtBookerSelect` / `rejectOnAwaitBooking` / `rejectOnAwaitBookingOpponent` / `rejectOnAwaitOpponentScheduleConfirm`：分别对应赛约等待 actionState，当前阶段达到系统配置的等待时长后取消比赛，全员回到 WAITING；不累计拒绝次数。
...
- `handleScheduleConfirm(matchId, userId, action, reasonCode?)`：非订场人处理赛约。
...
- `handleResultConfirm(matchId, userId, action, reasonCode?)`：
  - 确认：本人 CONFIRMED；全员 CONFIRMED → COMPLETED，以比赛表 winnerEntryNo 判断胜者并处理晋级（QUALIFY 胜者进 PAYING 待支付 / MAIN 胜者进下一轮 WAITING，交模块 5/晋级逻辑），败者 ELIMINATED。
...
选择订场人身份。入参 `matchId`，乐观锁防双方同抢，失败提示前端刷新。
...
处理赛约。入参 matchId + action（ACCEPT/REBOOK/REJECT）+ 可选 rebookReasonCode/rejectReasonCode。理由预设见源文档；终止性拒绝必须选理由。
...
处理结果。入参 matchId + action（CONFIRM/REJECT）+ 可选 rejectReasonCode。
