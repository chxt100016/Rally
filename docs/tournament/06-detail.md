...
## 领域 Service 能力（TournamentDetailService / action.state 计算）
- `assembleDetail(tournamentId, userId)`：并行装配各区块，userId 为空则只返回公开部分（tournament + progress）。
- **action.state 计算**（核心）：由后端根据 myEntry.status + myCurrentMatch.status + 是否订场人 + 各方确认状态**一次性算出**，取值见源文档枚举：NOT_LOGGED_IN / NOT_REGISTERED / NOT_REGISTERED_CLOSED / FROZEN / AWAIT_QUALIFIER_START / AWAIT_PAYMENT / AWAIT_COURT_BOOKER_SELECT / AWAIT_BOOKING / AWAIT_BOOKING_REBOOK / AWAIT_BOOKING_OPPONENT / AWAIT_SCHEDULE_CONFIRM / AWAIT_OPPONENT_SCHEDULE_CONFIRM / AWAIT_PLAYING / AWAIT_RESULT_SUBMIT / AWAIT_RESULT_CONFIRM / AWAIT_OPPONENT_RESULT_CONFIRM / WAITING_MATCH / IN_OFFLINE_STAGE / ELIMINATED / WITHDRAWN / END。
...
- action.state 是前后端契约核心：新增/变更比赛状态时须同步更新此计算逻辑。
