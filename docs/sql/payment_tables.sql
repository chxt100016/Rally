...
CREATE TABLE `payment_order` (
  ...
  `biz_type`               VARCHAR(32)  NOT NULL DEFAULT 'MEETUP_COLLECT' COMMENT '业务类型：MEETUP_COLLECT 活动收款/TOURNAMENT_ENTRY_FEE 赛事报名费（后续可扩展）',
  `collection_batch_id`    VARCHAR(32)  NOT NULL COMMENT '收款批次ID（一次「发起收款」生成一批支付单，共用同一批次；报名费场景为单单一批）',
  `meetup_id`              VARCHAR(32)  NOT NULL COMMENT '关联业务ID：MEETUP_COLLECT 为 rally_meetup.biz_id，TOURNAMENT_ENTRY_FEE 为 rally_tournament.biz_id',
  `payer_user_id`          VARCHAR(32)  NOT NULL COMMENT '付款人 user_id（参与者）',
  `payee_user_id`          VARCHAR(32)  DEFAULT NULL COMMENT '收款受益人 user_id（发起人），分账目标；报名费归平台不分账，留空',
