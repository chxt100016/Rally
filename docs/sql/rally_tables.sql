...
CREATE TABLE `sys_config` (
  ...
  `value_type`   ENUM('string','int','float','bool','json') NOT NULL DEFAULT 'string' COMMENT '值类型，决定读取时的解析方式',
...
CREATE TABLE `user_tennis_profile` (
  ...
  `status`            ENUM('tbc','normal','under_review') NOT NULL DEFAULT 'tbc' COMMENT '档案状态（裁定 D2）：tbc 未填写 / normal 正常 / under_review 核查期',
  ...
  `total_score`       DECIMAL(5,2) NOT NULL DEFAULT 0  COMMENT '总分 0~100 实时计算，由评分域写入',
  `rating_level`      ENUM('S','A','B','C') NOT NULL DEFAULT 'A' COMMENT '球友评级，新人初始 A（裁定 D14）',
...
CREATE TABLE `user_profile_change_log` (
  ...
  `type`         ENUM('ntrp','reputation','credibility','calibration','under_review') NOT NULL COMMENT '变更类型',
