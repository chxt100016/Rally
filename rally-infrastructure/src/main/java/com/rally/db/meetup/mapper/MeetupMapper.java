...
public interface MeetupMapper extends BaseMapper<MeetupPO> {
    ...
    List<MeetupPO> listPendingMeetups(@Param("userId") String userId, @Param("deadlineDays") int deadlineDays, @Param("lastId") String lastId, @Param("limit") int limit);

    /** RECENT tab：用户为创建人或已批准报名的约球，不限状态，searchAfter 游标分页 */
    List<MeetupPO> listRecentByUser(@Param("userId") String userId, @Param("lastId") String lastId, @Param("limit") int limit);
}
