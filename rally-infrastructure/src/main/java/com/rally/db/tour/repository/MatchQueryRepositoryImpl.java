...
public class MatchQueryRepositoryImpl implements MatchQueryRepository {
    ...
    @Override
    public List<MatchData> listFinishedByTournamentIdsAndDates(List<String> tournamentIds, Set<LocalDate> dates) {
        if (CollectionUtils.isEmpty(tournamentIds) || CollectionUtils.isEmpty(dates)) {
            return List.of();
        }
        List<TourMatchPO> list = matchService.lambdaQuery()
                .eq(TourMatchPO::getStatus, "FINISHED")
                .in(TourMatchPO::getTournamentId, tournamentIds)
                .in(TourMatchPO::getMatchDate, dates)
                .list();
        return list.stream().map(this::toMatchData).toList();
    }
