...
/**
 * FlareSolver 代理客户端，用于绕过 Cloudflare 人机验证。
 * 通过本地 FlareSolver 服务（Docker）发起请求，自动解决 JS 挑战。
 */
@Slf4j
@Component
public class FlareSolverClient {

    private static final String FLARESolver_URL = "http://localhost:8191/v1";
    ...
    /**
     * 通过 FlareSolver 发起 GET 请求，返回响应体 JSON 字符串
     *
    ...
    public String get(String targetUrl) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(FLARESolver_URL))
            ...
            if (responseStr == null) {
                log.error("FlareSolver 无响应, url={}", targetUrl);
                return null;
            }
            ...
            if (!"ok".equals(status)) {
                log.error("FlareSolver 请求失败, status={}, message={}, url={}",
            ...
            if (httpStatus != 200) {
                log.error("FlareSolver 目标返回非200, status={}, url={}", httpStatus, targetUrl);
                return null;
            }

            // FlareSolver 浏览器会将 API JSON 响应包裹在 <pre> 标签中，需要提取纯 JSON
            String response = solution.getString("response");
            return extractJson(response);

        } catch (Exception e) {
            log.error("FlareSolver 请求异常, url={}", targetUrl, e);
            return null;
        }
    }

    /**
     * 从 FlareSolver 返回的 HTML 中提取 JSON 内容。
