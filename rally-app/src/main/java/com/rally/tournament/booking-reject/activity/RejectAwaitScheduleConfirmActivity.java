...
/**
 * 业务活动 reject-await-schedule-confirm：订场人在对方确认超时后终止比赛并释放报名。
 */
@Component
@RequiredArgsConstructor
public class RejectAwaitScheduleConfirmActivity {
    ...
    public void execute(
        ...
        // A1-A3：由既有流程保留 SCHEDULED/订场人已确认/超时校验、版本保存及跨聚合结算语义。
        TournamentMatch rejectedMatch = rejectionSupport.requireMatch(matchId);
        rejectedMatch.rejectOnAwaitOpponentScheduleConfirm(bookerUserId, rejectReason);
