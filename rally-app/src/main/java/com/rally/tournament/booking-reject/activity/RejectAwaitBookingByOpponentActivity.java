...
/**
 * 业务活动 reject-await-booking-by-opponent：非订场方超时后终止等待并释放报名。
 */
@Component
@RequiredArgsConstructor
public class RejectAwaitBookingByOpponentActivity {
    ...
    public void execute(
        ...
        // A1-A3：既有流程保留 BOOKING/非订场方/超时校验、版本保存及跨聚合结算语义。
        TournamentMatch rejectedMatch = rejectionSupport.requireMatch(matchId);
        rejectedMatch.rejectOnAwaitBookingOpponent(opponentUserId, rejectReason);
        rejectionSupport.persistRejectedMatch(rejectedMatch);

        // A4：NotificationDeliveryService 在事务提交后异步触达，且通知失败在其内部容错。
        ...
        notificationDeliveryService.notify(
                ...
                otherParticipantIds(match, opponentUserId),
