...
public class TourTranslationService {
    ...
    public void matchGroups(List<MatchGroupDTO> groups, TranslationLanguageEnum language) {
        ...
        Map<TranslationKey, List<MatchGroupDTO>> map = new HashMap<>();
        collectGroupNames(groups, map, language);

        Map<TranslationKey, String> translationMap = this.translationQueryService.query(map.keySet());
        for (Map.Entry<TranslationKey, String> entry : translationMap.entrySet()) {
            map.get(entry.getKey()).forEach(g -> g.setName(entry.getValue()));
        }
    }
