...
public interface MatchAppConvertMapper {
    ...
    @Mapping(target = "status", expression = "java(com.rally.domain.tour.model.MatchStatus.toStatus(info.getStatus()))")
    @Mapping(target = "scheduledAt", expression = "java(parseDateTime(info.getMatchDate()))")
    @Mapping(target = "roundName", expression = "java(com.rally.domain.tour.model.TourRoundEnum.of(info.getRound().getLongName()))")
