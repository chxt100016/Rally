...
public class PlayerTournamentQueryService {
    ...
    public PlayerTournamentVO query(String tournamentId, Integer year, String playerId, String drawType) {
        // 1. 查询签表
        TennisDrawData draw = matchQueryGateway.getDrawByTournamentIdAndType(tournamentId, year, drawType);
        if (draw == null) {
            return null;
        }
        ...
        List<Long> tennisMatchIds = playerMatches.stream()
                .filter(m -> m.getTennisMatchId() != null)
                .map(MatchData::getTennisMatchId)
                .toList();
        List<SetScoreData> setScores = CollectionUtils.isEmpty(tennisMatchIds)
                ? List.of()
                : matchQueryGateway.listSetScoresByTennisMatchIds(tennisMatchIds);
...
//                vo.setScore("待定");
    ...
    private String formatScore(MatchData match, String playerId, Map<Long, List<SetScoreData>> setScoreMap) {
        if (!"FINISHED".equals(match.getStatus()) || match.getTennisMatchId() == null) {
            return "待定";
        }
        List<SetScoreData> sets = setScoreMap.getOrDefault(match.getTennisMatchId(), List.of());
