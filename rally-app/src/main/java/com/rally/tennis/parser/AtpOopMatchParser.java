...
@Component
public class AtpOopMatchParser extends MatchParser<Void, AtpOopResponse> {

    @Resource
    private TennisTvClient tennisTvClient;

    @Override
    public List<DrawResult<AtpOopResponse>> fetchDraws(Void params) {
        List<AtpOopResponse> response = tennisTvClient.getOop();
        if (CollectionUtils.isEmpty(response)) return List.of();

        List<DrawResult<AtpOopResponse>> results = new ArrayList<>();
        for (AtpOopResponse tournament : response) {
        ...
        for (AtpOopResponse.OopDay day : tournament.getOop()) {
            ...
            for (AtpOopResponse.CourtDetail court : day.getCourts().values()) {
                ...
                for (AtpOopResponse.MatchDetail detail : court.getMatches()) {
                    ...
                    if (detail.getMatchId() == null || !detail.getMatchId().startsWith("MS")) continue;
