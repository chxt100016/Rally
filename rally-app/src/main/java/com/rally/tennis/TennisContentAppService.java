...
public class TennisContentAppService {
    ...
    public String generateDailyContent(LocalDate date, TranslationLanguageEnum language) {
        List<TournamentData> tournaments = tennisTournamentGateway.findCurrentTournaments(date);
        ...
        List<Long> drawIds = matches.stream()
                ...
                .distinct()
                .toList();
        Map<String, Short> seedMap = tennisEntryGateway.listSeedMapByDrawIds(drawIds);
        ...
        List<PlayerData> players = tennisPlayerGateway.listByPlayerIds(new ArrayList<>(playerIds));
    ...
    public String generateSeedListContent(List<String> tournamentIds, TranslationLanguageEnum language) {
        ...
        List<TournamentData> tournaments = tennisTournamentGateway.listByTournamentIds(tournamentIds);
        if (CollectionUtils.isEmpty(tournaments)) {
            return "";
        }
        ...
        List<TennisDrawData> draws = tennisDrawGateway.listByTournamentIds(tournamentIds);
        if (CollectionUtils.isEmpty(draws)) {
            return "";
        }

        List<Long> allDrawIds = draws.stream().map(TennisDrawData::getId).toList();
        List<TournamentEntryData> allEntries = tennisEntryGateway.listByDrawIds(allDrawIds);
