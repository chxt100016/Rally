...
import com.rally.domain.user.model.MyProfileLevelDTO;
import com.rally.domain.user.model.MyProfileScoreDTO;
import com.rally.domain.user.model.MyProfileStatsDTO;
...
/**
 * 业务活动 assemble-my-profile：组装当前用户的完整本人档案。
 */
@Component
@RequiredArgsConstructor
public class AssembleMyProfileActivity {
    ...
    public MyProfileDTO execute(String userId) {
        // A1 保留原有查询及异常语义；只有 NORMAL/UNDER_REVIEW 视为完整档案。
    ...
    private MyProfileLevelDTO buildLevelDTO(UserProfile userProfile) {
        ...
        return new MyProfileLevelDTO()
                ...
                .setCanModify((cooldownDays == null || cooldownDays == 0)
                        && (remainingMatches == null || remainingMatches == 0));
    }

    /** A4 构建评分信息，评分明细权重及文案仍从系统配置读取。 */
    ...
    private List<ScoreItemDTO> buildScoreItemList(TennisProfileData profileData) {
        ...
        items.add(buildOneScoreItem("信誉分", "reputation", profileData.getReputationScore(),
                SystemConfigKey.SCORE_INFO_REPUTATION, 1));
        items.add(buildOneScoreItem("可信度", "credibility", profileData.getCredibilityScore(),
                SystemConfigKey.SCORE_INFO_CREDIBILITY, 2));
        items.add(buildOneScoreItem("校准度", "calibration", profileData.getCalibrationScore(),
                SystemConfigKey.SCORE_INFO_CALIBRATION, 3));
        return items;
    }

    private ScoreItemDTO buildOneScoreItem(String name, String key, Integer score,
                                            SystemConfigKey infoConfigKey, Integer sort) {
