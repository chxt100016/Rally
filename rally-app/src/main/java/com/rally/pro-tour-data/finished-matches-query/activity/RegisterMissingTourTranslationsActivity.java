...
/**
 * 业务活动 register-missing-tour-translations：逐项尽力登记完赛展示的缺译键。
 */
@Slf4j
@Component
...
public class RegisterMissingTourTranslationsActivity {
    ...
    private void registerOne(TranslationKey key) {
        try {
            ...
            // 通过 @content.translation-entry C1 创建空译文 PENDING；已有记录幂等成功且保留现有译文。
            ...
            // 并发唯一键竞争视为本项已登记，完赛响应继续使用原文。
            log.warn("并发登记完赛查询缺译键冲突，按已存在处理: entityType={}, originalText={}, language={}",
                    key.getEntityType(), key.getOriginalText(), key.getLanguage());
        } catch (Exception exception) {
            // 单项查询或保存失败只记录日志，已成功项不回滚，后续键继续。
            log.error("登记完赛查询缺译键失败: entityType={}, originalText={}, language={}",
