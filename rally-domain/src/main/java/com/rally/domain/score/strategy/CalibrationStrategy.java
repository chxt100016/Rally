...
public class CalibrationStrategy implements ScoreStrategy {
    ...
    public ScoreChange calculate(ScoreContext ctx, String userId) {
        ...
        // 5. 档位落分
        double t1 = SystemConfig.getFloat(SystemConfigKey.SCORE_CALIBRATION_DEVIATION_T1.getKey());
        double t2 = SystemConfig.getFloat(SystemConfigKey.SCORE_CALIBRATION_DEVIATION_T2.getKey());

        int score;
        if (deviation < t1) {
            score = SystemConfig.getInt(SystemConfigKey.SCORE_CALIBRATION_SCORE_UNDER_T1.getKey());
        } else if (deviation < t2) {
            if ("BELOW".equals(direction)) {
                score = SystemConfig.getInt(SystemConfigKey.SCORE_CALIBRATION_SCORE_BELOW_T1_T2.getKey());
            } else {
                score = SystemConfig.getInt(SystemConfigKey.SCORE_CALIBRATION_SCORE_ABOVE_T1_T2.getKey());
            }
        } else {
            if ("BELOW".equals(direction)) {
                score = SystemConfig.getInt(SystemConfigKey.SCORE_CALIBRATION_SCORE_BELOW_T2.getKey());
            } else {
                score = SystemConfig.getInt(SystemConfigKey.SCORE_CALIBRATION_SCORE_ABOVE_T2.getKey());
            }
        }
