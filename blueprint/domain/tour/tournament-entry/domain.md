...
| I2 | `entry_type` 沿用来源给出的非空原始值且最多 10 个字符，不做枚举归一化；seed 沿用来源解析结果，0 表示来源明确但不作为种子展示 | 参赛根、参赛资格 | 资格字段必须与现有采集和 `VARCHAR(10)` 存储边界一致 | `TOUR_ENTRY_QUALIFICATION_INVALID` |
...
|---|---|---|---|---|---|
| C1 | 新增或刷新参赛资格 | 同身份不存在或 `CONFIRMED` | drawId、playerId、可选 seed/entryType | 新建 `CONFIRMED`，或非空合并资格且状态不变 | 身份缺失/冲突；entryType 超过存储上限；已退出 |
...
- entryType 保留 ATP/WTA 来源短码或原始文本，不把 `WC/Q/LL` 扩展为长名称；超过 10 个字符时不能持久化。
...
`uk_tour_entry_player_draw_year` 实际保护 `draw_id+player_id`。采集实现只调用 `C1` 并限制更新列为 seed/entry_type；entryType 原样传递，不增加主线不存在的枚举转换。退出必须走独立 `C2`。按 playerId 查询职业球员时还需从签表取得 tour，避免跨巡回赛歧义。
