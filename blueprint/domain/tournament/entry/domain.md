...
| I3 | 地区和可比赛时间集合均至少一项，courtAbility 只接受 `CAN_BOOK/CANNOT_BOOK`；列表元素按输入原样保留，不清洗、去重或做格式校验；三组偏好必须整组替换 | 报名根、匹配偏好 | 匹配算法只能消费一份同版本、完整的约束集合 | `TOURNAMENT_ENTRY_PREFERENCE_INVALID` |
...
| C2 | 整组替换匹配偏好 | 非 `CHAMPION/ELIMINATED/WITHDRAWN` | 地区集合、courtAbility、时间集合 | 状态与进度不变，三组偏好一起替换并原样保留列表元素 | 地区或时间集合缺失/为空；courtAbility 缺失或非法；报名已终止 |
...
| C6 | 结算比赛结果 | 调用活动已确认比赛完成；报名命令本身不校验原状态必须为 `IN_MATCH` | 胜负、比赛轮次及完成时间 | 资格胜方 `PAYING`、资格负方 `WAITING`；非决赛正赛胜方 `WAITING` 并晋级，负方 `ELIMINATED`；决赛胜方 `CHAMPION` | 胜方缺失；赛段或轮次无法推进 |
...
- 比赛完成活动在事务中先确认比赛事实再调用报名推进；报名根的胜负推进方法不复核原状态，异常存量状态也会按赛段和比赛轮次被覆盖。
