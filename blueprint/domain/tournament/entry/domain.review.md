...
- [Q10] 未入赛淘汰是否新增独立运营命令，允许 WAITING/FROZEN 进入 ELIMINATED？
  > 是。新增 C11 运营淘汰命令，允许 WAITING 或 FROZEN 迁移到 ELIMINATED。
  → 落入状态、命令与边界情况：新增 C11 WAITING/FROZEN→ELIMINATED。

- [Q11] 报名淘汰命令是否要求 currentRound 等于调用方传入的赛事当前轮次？
  > 是。命令接收调用方确认的赛事当前轮次，报名 currentRound 必须相等。
  → 落入不变量和命令：要求报名轮次等于预期赛事当前轮次。

- [Q12] 跨 entryNo 整组一致性是否仍由活动事务协调，单报名聚合只守护自身条件迁移？
  > 是。单报名聚合只守护自身状态与轮次条件；完整 entryNo、多成员和无在途比赛由领域服务预检及活动事务复核协调。
  → 落入边界、不变量与实现提示：单根条件迁移，整组由活动协调。
