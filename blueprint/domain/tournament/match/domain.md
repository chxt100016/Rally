...
| 状态 | 含义 | 可迁移到 | 触发命令 |
|---|---|---|---|
| `MATCHED` | 已组成对阵，尚未选订场人 | `BOOKING/REJECTED` 或物理删除 | `C2/C8/C9/C10` |
...
| I8 | 按 `tournamentId+matchNo` 运营终止时必须锁定并使用最新根；`COMPLETED` 不得终止，首次终止只把根状态改为 `REJECTED` 并递增版本，保留根其他字段和全部参与者；已为 `REJECTED` 时幂等返回当前联动快照 | 比赛根、全部参与者、比赛身份 | 后续赛约和报名联动依赖同一份当前参与快照，且终止状态、版本与历史数据保留必须由一次聚合命令保证 | `TOURNAMENT_MATCH_CANCEL_FORBIDDEN` / `TOURNAMENT_MATCH_VERSION_CONFLICT` |
...
| C10 | 运营终止指定未完成比赛 | `MATCHED/BOOKING/SCHEDULED/PENDING_PLAY/PENDING_CONFIRM/REJECTED` | tournamentId、正数 matchNo、运营终止意图 | 按自然键锁定最新根并生成联动快照；首次终止将状态改为 `REJECTED`、version+1，其他字段与参与者不变；已为 `REJECTED` 时幂等不写 | 目标不存在；最新状态为 `COMPLETED`；首次终止条件更新未命中 |

## 边界情况

- 恰有一个可订场参与者的匹配可直接创建为 BOOKING；否则从 MATCHED 等待首次认领。
- BOOKING 提交赛约会重置赛约确认；SCHEDULED 内只修改赛约资料时保留已确认结果。
- SCHEDULED 内修改只保存外部赛约，不写比赛根或参与者，也不比较或递增比赛 version。
...
- C10 首次按自然键查不到目标与首次终止条件更新未命中分档处理；成功快照包含 tournamentId、matchId、matchNo、可选 meetupId 以及全部参与者 userId/entryNo，参与者为空时返回空列表。已为 REJECTED 时不重复递增版本，但仍返回当前快照。
...
两个唯一键分别保护业务 id 和赛事内编号，`uk_match_user` 保护根内参与者唯一。仓储必须整聚合加载并批量保存参与者；所有阶段命令使用 version 条件更新根，受影响行数为 0 即冲突。C10 使用 `tournament_id+match_no` 锁定读取最新根；首次终止以 `biz_id+version+status NOT IN (COMPLETED,REJECTED)` 条件只更新 `status=REJECTED, version=version+1`，参与者和其他根字段不写，已为 REJECTED 时直接返回快照。补齐状态枚举中的 `PENDING_PLAY`，不要只依赖旧数据库列注释。
