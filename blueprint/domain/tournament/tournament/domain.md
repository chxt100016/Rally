...
| 运行进度 | 已锁定正赛席位、当前轮次、线下活动 id | `current_filled_slots`、`current_round`、`offline_meetup_id` |
...
| `ACTIVE` | 已开放报名、匹配和比赛 | `ABANDONED` | `C4` |
| `ABANDONED` | 运营废弃的终态 | `ABANDONED` | 无 |

现有模型没有 COMPLETED 状态；FINAL 完成、endTime 和冠军收口不由本聚合现有命令自动推导。
...
| I3 | offlineFromRound 必须是受支持轮次且其 slotCount 严格小于 totalSlots；offlineMeetupId 只能在 currentRound 等于它时从空绑定一次 | 赛事根、赛制配置、运行进度 | 防止在线阶段未到即创建或并发绑定多个线下活动 | `TOURNAMENT_OFFLINE_ROUND_INVALID` |
...
|---|---|---|---|---|---|
| C1 | 创建赛事草稿 | 不存在 | bizId、完整合法配置及解析后的 cityName | `DRAFT/QUALIFIER`，锁位 0，endTime/offlineMeetupId 为空 | 配置、金额、时间、城市或身份非法 |
| C2 | 更新赛事配置 | `DRAFT/ACTIVE/ABANDONED` | 完整配置意图；可选 null 表示保留旧值 | 状态与全部运行进度不变，仅原子替换合法配置 | 赛事不存在；配置无效；换城未同步名称；试图覆盖进度 |
...
- offlineFromRound 沿用完整轮次枚举，只要 slotCount 小于 totalSlots；因此总签位 2 时只有 QUALIFIER 满足当前规则。
- 配置更新允许发生在 ACTIVE/ABANDONED，但不得改变 status、currentRound、currentFilledSlots、endTime 或 offlineMeetupId。
...
- 决赛完成仍可保持 ACTIVE/FINAL；本轮不臆造赛事完成或冠军状态。

## 实现提示

配置更新使用显式白名单 DTO，绝不映射运行进度列；城市变更先调用 location-catalog。锁位 SQL 带 `current_filled_slots < total_slots` 条件并检查影响行数，轮次更新使用固定枚举顺序条件。线下绑定使用 `offline_meetup_id IS NULL` 条件更新以收敛并发。
