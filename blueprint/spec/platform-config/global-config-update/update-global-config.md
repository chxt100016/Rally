...
3. 拒绝长度超过 100,000 个 Java 字符的内容。`home.page.config` 必须是根对象且包含有序 `sections` 数组，按区域、海报区块可见城市、单张海报和海报导航占位符的整体结构校验，并重新序列化为紧凑 JSON。可选 `cityId` 只作为 `POSTER` 区块字段校验；旧的三项首页 key 不再属于配置名录，旧 `cityAware` 和单张海报上的旧 `cityId` 作为未识别扩展字段原样保留但不产生规则效果。
...
6. 记录已存在时，校验持久化身份、`valueType` 和提交版本，但不按当前名录重新校验即将被覆盖的旧内容；随后以记录编号和提交版本做条件原子更新。成功后覆盖为已经通过当前名录校验的新值、同步当前名录说明、重新启用并将版本加 1，但不重新写入 `valueType`。版本不匹配或记录被并发改动时拒绝。
...
| `PARAM_ERROR` | 完整首页 JSON 无效，区域、标题、海报数量、标识、类型、图片、交互方式或海报区块城市 id 不符合规则，或海报跳转目标含未登记、空白、未闭合或无配对的占位符 | publish-global-config | 不产生配置变更 | 配置格式无效，或对应的具体首页校验提示 |
...
首页最多 30 个区域；区域 `id` 必须非空、不重复，且仅含字母、数字、下划线或中划线，最长 64 位。区域类型限于 `MEETUP`、`TOUR_MATCH`、`POSTER`、`NEWS`，除 `POSTER` 外同类只能一个。`POSTER` 区域必须有非空标题和海报数组；区块 `cityId` 可缺失、为 `null` 或空白字符串，非空时必须是字符串但不校验是否已进入城市名录。每区最多 20 张海报；单张海报的 `actionType` 限于 `NAVIGATE` 或 `PREVIEW`，图片 key 必填，单张海报上的旧 `cityId` 不再作为已识别字段。`wechatUrl`、`appUrl`、`webUrl` 可选字符串，只有这三个字段允许出现海报导航占位符；当前仅登记 `{{cityCode}}` 与 `{{cityName}}`，旧 `{{cityId}}` 占位符和任何其他 `{{...}}`、空占位符、未闭合 `{{` 或无配对 `}}` 均拒绝发布。
...
- 海报可见城市：只校验 `POSTER` 区块的可选 `cityId` 字符串，不校验城市名录；单张海报的旧 `cityId` 不再读取
