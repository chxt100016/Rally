...
成功返回 `configs` 列表，按已登记顺序包含更新后的 64 项配置；每项包含 `key`、`description`、`configValue`、`defaultValue`、`version` 和 `overridden`。
...
3. 拒绝长度超过 100,000 个 Java 字符的内容。三项首页 JSON 配置按各自结构、数量、类型和必填文本规则校验，并重新序列化为紧凑 JSON。
...
8. 按已登记名录重新查询并交付 64 项配置的当前值、默认值、版本和覆盖状态。任一后续步骤失败会使数据库事务回滚，但已重建的进程内缓存不受事务回滚保护。
...
| `PARAM_ERROR` | 首页 JSON 无效，或区域、标题、海报数量、标识、类型、图片不符合规则 | publish-global-config | 不产生配置变更 | 配置格式无效，或对应的具体首页校验提示 |
...
首页布局最多 30 个区域；区域 `id` 必须非空、不重复，且仅含字母、数字、下划线或中划线，最长 64 位。区域类型限于 `MEETUP`、`TOURNAMENT_POSTER`、`TOUR_MATCH`、`COURT_POSTER`、`POSTER`、`NEWS`，除 `POSTER` 外同类只能一个。海报每区最多 20 张，类型限于 `NAVIGATE` 或 `PREVIEW`，图片 key 必填；自定义海报区还要求标题，赛事海报配置要求标题和副标题。
...
- 发布：`HomeConfigAdminAppService.updateAny()` / `save()`，`@Transactional`
- 校验：`validateAndNormalize()` / `validateScalar()` / `validateHomeSections()` / `validatePosters()`
- 并发更新：`SysConfigService.updateValueIfVersion()`，条件 `id + version`
- 缓存刷新：`SystemConfig.init()`，只作用于当前 JVM
- 成功响应：`HomeConfigAdminAppService.getAll()`，返回 64 项
