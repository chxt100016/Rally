...
- `POSTER_CARD`：标题、副标题和海报数组；海报包含 `type`、`imageUrl`、标题、副标题及微信/应用/网页导航值。
...
- query-home-poster-section  交付赛事、城市球场或自定义海报区域
...
flowchart TD
    ...
    D -->|TOURNAMENT_POSTER / COURT_POSTER / POSTER| P[query-home-poster-section 查询海报区]
...
2. 从当前运行实例的 `home.layout.config` 读取 JSON 数组；解析抛出异常时记录日志并回退枚举默认布局。按数组顺序遍历，对象为空或 `enabled=false` 时跳过；`enabled` 缺失或非明确 `false` 均视为启用。
3. 区域 `type` 为空或不是 `MEETUP`、`TOURNAMENT_POSTER`、`TOUR_MATCH`、`COURT_POSTER`、`POSTER`、`NEWS` 时省略。每个区域的构建异常都单独捕获并记录，省略该区域后继续处理后续配置。
...
5. `TOURNAMENT_POSTER` 区域读取 `home.tournament.poster.config`；解析异常时回退该项默认 JSON。区域非空标题和副标题优先，否则使用海报配置中文案；海报不附加城市信息。
6. `COURT_POSTER` 区域使用配置标题，空白时为“附近球场”；副标题空白时用城市名录组成“寻找「<城市名>」的球场”。然后读取 `home.poster.config` 并为每个非空导航目标附加本次城市参数。
7. `POSTER` 区域直接使用区域内的标题、副标题和海报数组。仅当 `cityAware=true` 时为非空导航目标附加城市参数。
8. 海报按数组顺序转换：交互类型必须可转为 `NAVIGATE` 或 `PREVIEW`；图片 key 为空白时 URL 为 `null`，否则签发一小时七牛 URL；文案和三端导航值原样取值。任一海报转换失败都终止该数组的后续处理，保留失败前已形成的海报。
9. 城市感知导航值非空时，无论原 URL 是否已有查询参数，都直接追加 `?cityCode=<code>&cityName=<name>&mode=view`，不做 URL 编码。未知城市导致球场海报区构建异常并被省略；城市感知自定义海报保留区域，但在首个需附加城市的导航值处停止后续海报。
10. `TOUR_MATCH` 区域查询开始日不晚于明天且结束日不早于昨天的赛事，过滤数字级别小于 250 的记录，保留空白或非数字级别。将日期重叠且城市名忽略大小写相同的赛事跨巡回赛合并，组内和组间按 `GS`、`1000`、`500`、`250`、其他级别及日期/编号排序，每组首项作为代表赛事。
11. 对每个赛事组查询未结束比赛，并补入这些比赛日期内已结束的比赛；双方球员都未确定的记录被过滤，单方确定则保留。按日期和球场分组后，首页只取最早日期组的排序首个球场及其全部比赛；缺少赛事编号、日期、球场或可展示比赛的组被省略。
12. 对已形成的赛事名、球场名和球员名查询简体中文缓存；有非空译文时替换，未命中时保留原文并尝试新建待翻译记录，单条保存失败只记录日志。已形成赛事记录数用于拼接“<数量>场ATP／WTA、进行中”副标题，不是比赛场次。
13. `NEWS` 区域使用配置标题和副标题，空白时分别回退“资讯”和“最新动态”，`newsItems` 始终为空列表。
14. 仅将成功构建的区域按原配置顺序加入 `displayItems`。没有可交付区域时返回空列表；本查询不修改首页配置或各内容源，但可因翻译缓存未命中而新增待翻译记录。
...
| —（默认配置） | 布局、赛事海报或通用海报的当前 JSON 解析抛出异常 | 对应区域查询 | 记录日志并用该配置的枚举默认 JSON 继续 | 无 |
...
| —（部分海报） | 海报对象为空、类型无效、图片签名失败，或城市感知导航无法取得城市名 | query-home-poster-section | 终止当前海报数组的后续处理，保留已形成海报；若失败发生在球场区副标题构建，则省略整个区域 | 无 |
...
| `SYSTEM_ERROR` | 布局解析得到 `null` 等无法遍历的结果，默认布局也无法解析，区域对象提取在区域保护块之外失败，或发生其他顶层未处理异常 | 流程编排 | 终止整个首页请求，不返回已形成区域。本流程无整体事务；在失败前已新建的待翻译记录不会回滚 | 系统异常，请稍后重试 |
...
- 顶层组装：`HomeAppService.getHomePage()` / `buildDisplayItem()`
- 布局与海报配置：`home.layout.config`、`home.tournament.poster.config`、`home.poster.config`
