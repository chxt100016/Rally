---
id: social-relations.followers-list.activity.query-followers-page
depends_on: []
reads:
  - name: user_follow
...
sequenceDiagram
    ...
    participant A as query-followers-page 活动
    participant DB as 关注、用户与网球档案
    C->>A: userId/lastId/size
    A->>DB: 查询 size+1 条粉丝关系
    A->>DB: 批量补资料与当前用户关注状态
    A-->>C: PageDTO
```
...
2. 按 followingId 查询；lastId 非空白时加 `bizId < lastId`，按 bizId 倒序取 size+1，多取一条判 hasMore。
3. 返回最多 size 条，total/nextCursor=null；每项 cursor=bizId，调用方取末项继续。
4. 按 followerId 批量补昵称、头像、NTRP；资料缺失仍保留 userId/cursor，展示字段为空。头像键非空生成 3600 秒签名地址。
5. isFollowed 表示当前登录用户是否关注本页粉丝，不是名单所属用户是否回关。
