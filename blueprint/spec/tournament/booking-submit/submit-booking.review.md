...
- [Q4] SCHEDULED 内修改赛约是否继续保留全员原确认结果且不重复发送订场通知？
  > 是。按 main 保留原确认结果，且仅 BOOKING 提交后发送订场通知。
  → 已落入详细流程第 5、6 步与技术线索。

- [Q5] BOOKING 状态未传 meetupId 时是否继续允许新建赛约并覆盖比赛关联？
  > 是。维持现有兼容行为，不新增必须复用原赛约的限制。
  → 已落入详细流程第 3 步与服务边界。

- [Q6] 请求 tournamentId 是否继续只做必填校验，业务归属仍采用比赛记录？
  > 是。保持现有接口与业务归属口径，不新增一致性校验。
  → 已落入接口契约与详细流程第 2 步。
