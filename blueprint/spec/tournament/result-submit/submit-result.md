...
请求体必须包含非空 `matchId` 和 `winnerEntryNo`，`acceptedNoticeScenes` 可选。成功返回无数据响应。
...
1. 识别当前登录用户，接收非空比赛编号、获胜方参赛编号与可选通知授权。
...
| 无对外失败 | 授权场景空、无效或登记失败 | submit-result | 忽略无效值或记录异常，不影响赛果 | 提交成功 |
...
- 事务：应用与领域流程均 `@Transactional(rollbackFor = Exception.class)`；通知授权登记内部容错
