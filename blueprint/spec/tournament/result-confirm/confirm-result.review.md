...
- [Q4] 本人已 CONFIRMED 时是否继续允许重复确认并刷新确认时间？
  > 是。保持现有确认逻辑，不增加必须为 PENDING 的条件。
  → 已落入详细流程第 3 步与业务边界。

- [Q5] 冠军结算是否必须以已完成比赛自身 round=FINAL 判断，而非赛事 currentRound？
  > 是。仅比赛自身为 FINAL 时产生冠军并结束赛事，避免误把其他轮次当决赛。
  → 已落入详细流程第 5、6 步。

- [Q6] 资格赛胜方是否继续只进入 PAYING，不在本服务占用正赛席位？
  > 是。资格赛胜方只进入 PAYING，支付与席位锁定由独立服务承担。
  → 已落入详细流程第 5 步与业务边界。
