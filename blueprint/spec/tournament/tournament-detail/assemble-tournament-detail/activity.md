...
| `entryOptions` | 报名可用选项 | 是 | `districtOptions` 为赛事城市完整区县级行政区划的中文名称，按区县编码升序；`timeOptions` 固定依次为工作日白天、工作日晚上、周末上午、周末下午、周末晚上 |
...
| 赛事城市无法识别 | 赛事城市未在行政区划名录中命中 | get-tournament-detail 流程赛事城市无法识别一行 |
...
- A1 聚合赛事、报名可用选项、冠军成员身份与整体进度，统计当前轮次的可晋级与已晋级名额。
- A2 按身份裁剪个人区块。
- A3 补当前比赛和对手信息。
- A4 推导当前动作与活动卡片。
...
1. A1 读取赛事、全部报名、比赛和参与关系，组装公开资料、展示状态、进度、签表、参赛者和拒赛统计；按赛事城市取得完整区县级行政区划，依区县编码升序映射为中文区域选项，并补充固定的五项可比赛时间。城市无法识别时区域为空且不使用其他城市代替，时间选项仍正常交付。冠军参赛编号为空时交付空 `championUsers`；否则从该参赛编号的报名记录中依次收集用户编号与搭档编号，保留首次出现顺序并按用户编号去重。资格赛可晋级名额等于正赛总签位，已晋级名额按已支付并进入正赛的参赛编号去重统计；正赛各轮的可晋级名额等于本轮参赛签位的一半，已晋级名额等于当前轮次已完成比赛数，决赛因此为 1 个可晋级名额且在完赛后为 1 个已晋级名额；调用方用可晋级名额减已晋级名额得到剩余名额。
...
- 报名可用选项不按访问者身份裁剪；行政区划名录命中但没有下属区县时，区域同样为空列表。
- 详情查询本身不改变赛事、比赛或报名状态。
