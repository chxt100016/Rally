...
路径参数 `bizId` 指定赛事。接口允许匿名访问，成功返回赛事聚合详情；返回内容按登录、报名和当前比赛状态裁剪。赛事进度中的当前轮次可晋级名额表示本轮胜出席位总数，已晋级名额表示已产生的胜出席位，剩余名额表示两者之差；决赛以 1 个冠军名额计。赛事图片与用户头像地址限时一小时。
...
5. 根据赛事时间、本人报名、报名轮次与赛事当前轮次、当前比赛和关联赛约推导当前动作；冠军报名显示 `CHAMPION`，已晋级但赛事尚未整体推进时显示 `ADVANCED`，进入当前轮匹配池后才显示 `WAITING_MATCH`。关联赛约或线下活动存在时压缩成交付卡片。
