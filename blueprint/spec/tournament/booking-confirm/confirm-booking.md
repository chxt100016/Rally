...
```mermaid
flowchart TD
    A[confirm-booking 确认赛约] -->|尚有未确认者| W([保持 SCHEDULED])
...
3. 将本人赛约确认改为 `CONFIRMED` 并记录当前时间，以版本条件保存比赛与参与关系。
4. 若尚有未确认者，比赛保持 `SCHEDULED`；若全员确认，将比赛改为 `PENDING_PLAY`。
5. 全员确认时，若关联赛约存在且为 `DRAFT`，将其改为 `OPEN`；其他情况不阻止比赛进入待比赛。
...
- 调用：`TournamentMatchAppService.confirmSchedule()` → `TournamentMatchFlowService.handleScheduleConfirm()` → `TournamentMatch.confirmSchedule()` → `activateDraftMeetup()`
