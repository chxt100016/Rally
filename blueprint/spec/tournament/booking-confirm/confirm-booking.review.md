...
- [Q4] 是否仅在关联赛约记录存在时校验开始时间，关联缺失继续兼容放行？
  > 是。仅存在关联赛约记录且开始时间严格早于确认时间时拒绝，恰等时继续；缺失时保持现有兼容行为。
  → 已落入详细流程第 3 步与异常分支。

- [Q5] 本人已 CONFIRMED 或为 REJECTED 时是否继续允许覆盖为 CONFIRMED 并刷新时间？
  > 是。保持现有确认覆盖逻辑，不增加必须为 PENDING 的前置条件。
  → 已落入异常分支说明。

- [Q6] 全员确认但赛约编号或记录缺失时，比赛是否仍进入 PENDING_PLAY？
  > 是。全员确认后比赛照常进入 PENDING_PLAY，仅不创建或修改缺失的赛约活动。
  → 已落入详细流程第 5、6 步与异常分支。
