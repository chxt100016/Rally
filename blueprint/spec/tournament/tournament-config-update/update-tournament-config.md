...
2. 按业务编号读取赛事，不存在则拒绝；执行与创建相同的签位、线下轮次、组人数、费用、奖金和时间关系校验。
3. 将名称、图片键、主题、比赛类型、城市编码、NTRP、性别、签位、线下轮次、组人数、费用、奖金、时间、拒赛上限和规则映射到存量对象。
4. 映射器会把命令空值写入内存对象，但后续 MyBatis-Plus 实体更新忽略空列，因此可选配置传空不能清除数据库旧值；城市编码可变但不会重新查询或同步城市名称。
5. 保留赛事状态、当前轮次、已锁定席位、结束时间、线下活动关联及其他运营数据，不联动报名、比赛、支付或匹配。
...
更新不限制赛事状态。可选字段传空会因实体更新策略保留数据库旧值；城市编码变更后城市名称仍为旧值。
