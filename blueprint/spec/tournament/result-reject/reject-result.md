...
请求体含非空 `matchId`、`confirm=false` 和非空 `rejectReason`，`acceptedNoticeScenes` 可选。成功返回无数据响应。
...
1. 识别当前登录用户，接收比赛编号、`confirm=false`、拒绝理由和通知授权。
...
6. 登记本次有效通知授权，并在事务提交后向其他参与者尝试发送赛事拒绝通知，返回成功。
...
| 无对外失败 | 关联赛约不存在/非 `DRAFT`，参与者报名非 `IN_MATCH`，或授权/通知失败 | reject-result | 不修改不符状态的对象；通知失败记录后容错 | 拒绝成功 |
...
- 事务：`@Transactional(rollbackFor = Exception.class)`；通知登记内部容错，通知在提交后异步发送
