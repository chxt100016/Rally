...
球场对象列表；每项包含编号、名称、别名、地址、经纬度、城市区域、环境及展示名、标签、备注和可解析的扩展资料。现有转换不把 `meetup_count` 映射到对外 `total`，该字段保持为空；无匹配记录时为空列表。
...
4. `A3` 从 `ext_data` 补充拼音、拼音首字母、评分、费用、开放时间和联系电话，再映射其余基础字段；沿用现有转换，`meetup_count` 不映射到对外 `total`。
...
- 即使存储记录存在 `meetup_count`，对外 `total` 仍按主线转换行为保持为空。
