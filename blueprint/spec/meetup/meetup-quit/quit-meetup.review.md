...
- [Q5] 约球发布者本人是否继续允许通过本服务退出自己的普通约球？
  > 是。保持现有代码行为，不新增发布者退出限制。
  → 已落入详细流程第 3、4、8 步。

- [Q6] 退出是否继续不按约球状态或当前时间增加限制？
  > 是。普通约球退出只校验约球性质和报名状态，不新增生命周期或时间限制。
  → 已落入详细流程第 2 步与服务边界。

- [Q7] REVIEWED 与 SKIPPED 报名是否继续允许转为 QUIT？
  > 是。JOINED、REVIEWED、SKIPPED 均继续允许转为 QUIT。
  → 已落入详细流程第 3 步与异常分支。
