...
- [Q5] 每日发布上限是否继续只统计大写 OPEN 与小写 full？
  > 是。按 main 现有口径保留，不扩大计数状态集合。
  → 已落入详细流程第 2 步、异常分支与技术线索。

- [Q6] TEXT 或 MAP 引用不到球场时是否继续用请求资料发布？
  > 是。保持降级逻辑，不因球场编号失效新增失败分支。
  → 已落入详细流程第 5 步与异常分支说明。

- [Q7] 发布成功是否继续不返回新约球编号或详情？
  > 是。保持现有接口契约，只返回成功标识。
  → 已落入接口契约与详细流程第 10 步。
