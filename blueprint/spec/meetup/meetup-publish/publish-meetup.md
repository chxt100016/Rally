...
| `acceptedNoticeScenes` | 字符串列表 | 否 | 所有可识别场景可登记，非法项忽略、重复不去重 |

### 成功响应

无业务数据；成功表示约球、发布者报名和群聊成员已提交。接口不交付新约球编号、详情或通知授权结果。

## 业务活动

- create-open-meetup  校验发布资格与内容，建立开放约球及发布者已加入报名
- join-publisher-chat  为发布者建立初始未读数为零的约球群聊成员关系
- grant-publisher-notification-quota  按本次可识别授权场景尽力登记通知额度
...
flowchart TD
    ...
    B --> C[grant-publisher-notification-quota 登记授权额度]
    C -->|登记失败| S([保留发布并返回成功])
    C --> S
```
...
10. 解析本次订阅授权场景并尽力登记额度；非法场景忽略，所有可识别场景均可登记且重复不去重。登记失败不回滚发布。
11. 返回发布成功，不交付新约球编号或详情，也不在本流程发送通知。
...
球场编号未命中时按请求场地继续发布。命中球场库时，新约球会重新生成编号并按城市名录写城市名，但当前沿用球场记录的创建与更新时间。通知额度登记失败被吞掉并记录日志，不回滚发布，也不向发布者反馈。
...
- 当日计数状态：`OPEN`、小写 `full`
- 场地模式：`TEXT`、`MAP`、`FREE`
- 通知授权：所有可识别 `NoticeScene`，只登记、不发送
