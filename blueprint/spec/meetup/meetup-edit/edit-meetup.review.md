...
- [Q5] 非发布者是否继续在赛事阶段、编辑锁定和城市规则之后才被拒绝？
  > 是。为保持现有业务行为和失败码顺序，继续先执行赛事阶段、编辑锁定、城市与成员锁定校验，再校验发布者。
  → 已落入详细流程第 4 步与异常分支。

- [Q6] TEXT 或 MAP 指定球场不存在时是否继续降级使用请求场地资料？
  > 是。继续按现有逻辑降级采用请求中的场地名称、地址和坐标，不新增失败分支。
  → 已落入详细流程第 5 步与异常分支说明。

- [Q7] 命中球场库时是否按现有代码保留球场编号及时间字段覆盖行为？
  > 是。本轮重构不修正该映射行为，文档保留风险说明，代码保持主分支现状。
  → 已落入详细流程第 8 步、异常分支与技术线索。
