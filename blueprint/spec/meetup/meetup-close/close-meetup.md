...
- dispatch-meetup-cancel-notifications  在事务提交后按可用订阅额度异步通知仍有效的参与者
...
flowchart TD
    ...
    G -->|无额度、已退出或发送失败| H[跳过或记录失败，不回滚关闭]
```
...
6. 事务成功提交后，以除发布者外的有效参与者为候选，异步查找每人最早一条可用的约球取消通知额度。
7. 发送前再次校验候选人仍具备通知资格；已退出者跳过且保留额度，其他候选人通过微信接收约球名称、时间、地点和“创建人取消”。
8. 通知缺额、查询失败、渠道缺失或发送失败只记录结果，不改变已经提交的关闭结果；接口不等待异步通知完成即返回成功。
...
通知阶段不再改变接口结果：没有可用额度时不发送；发送前确认已退出则跳过并保留额度；通知资格校验异常时按可发送处理；渠道缺失、微信身份缺失、微信发送或通知数据读写失败时记录失败或日志，不回滚 `CLOSED`。

## 技术线索

- 表 `meetup` 与报名记录：读取发布者、约球类型、时间、状态、当前人数和有效参与者并保存 `CLOSED`
- 系统配置 `meetup.cancel.penalty_*`：按距开始时间计算四档处罚，目前仅记录日志
- 订阅通知流水：事务提交后查询并占用每位候选人最早一条可用额度
