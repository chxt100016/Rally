...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P2 | 两个入口对同一删除承诺交付不同结果：档案视频管理入口返回更新后的个人档案，上传资源管理入口只返回成功确认；需产品负责人统一删除成功的对外交付物。 | 产品与用户运营负责人 |
| P1 | 两个入口的视频归属规则不一致：上传资源管理入口要求 `key` 位于本人前缀下，档案视频管理入口可提交任意七牛云存储标识并尝试删除；需产品与安全负责人统一归属校验。 | 产品与用户运营负责人 |
| P1 | 两个入口都不确认 `key` 已登记在本人档案中；未登记时仍会删除七牛云同标识文件并返回成功。需产品负责人确定“未找到档案视频”应视为已经达到删除目标还是明确失败，并由安全负责人确定可删除资源范围。 | 产品与用户运营负责人 |
| P1 | 两个入口的入参校验不一致：档案视频管理入口拒绝空白 `key`，上传资源管理入口把空白值判为无权操作、缺少参数则返回系统异常；需产品与技术负责人统一参数规则和提示。 | 产品与用户运营负责人 |
| P1 | 档案缺失时两个入口分别返回系统异常和“档案不存在”；需产品负责人统一无档案用户的业务提示。 | 产品与用户运营负责人 |
| P1 | 删除按 `key` 移除所有匹配项；若多个列表项共用同一 `key`，删除前虽多于一项，删除后仍可能变成空列表。需产品负责人确定存量重复项的处理规则，并保证至少保留一个视频的承诺。 | 产品与用户运营负责人 |
| P1 | 档案视频管理入口删除七牛云文件后还要汇总完整个人档案；若后续汇总失败，档案列表恢复原值但文件已经删除。需产品与技术负责人确定一致性修复和对外结果。 | 产品与用户运营负责人 |
| P2 | `TBC` 档案可以删除视频，但档案视频管理入口成功结果不展示删除后的视频列表；需产品负责人确定待完善阶段是否允许使用本服务及成功后的交付内容。 | 产品与用户运营负责人 |
| P1 | 七牛云文件不存在时固定按删除成功处理；需产品负责人确认这一幂等口径是否作为正式承诺。 | 产品与用户运营负责人 |
| P2 | `user_tennis_profile.status` 的建表说明使用小写 `tbc/normal/under_review`，当前业务读取和保存使用大写 `TBC/NORMAL/UNDER_REVIEW`；需技术负责人统一存储取值并确认历史数据兼容方式。 | 产品与用户运营负责人 |
| P2 | 返回档案中的“系统建议”固定声称依据近 20 场对战数据评估为 4.5，当前没有读取或计算相应数据；需产品负责人确定真实计算依据和展示条件。 | 产品与用户运营负责人 |
| P2 | 返回档案固定提示手动修改后进入 90 天冻结期，但实际冻结期按可信度选择可配置的低、中、高三档；需产品负责人统一对外口径。 | 产品与用户运营负责人 |
| P2 | 可信度低于 30、低于 60 和不低于 60 的冻结期档位边界写死在代码中；需产品负责人确定档位边界。 | 产品与用户运营负责人 |
| P1 | 启用的数字配置无法识别时按 `0` 使用且仍返回成功，可能造成视频限制、冻结期限或评分失真；需产品负责人确定异常配置的处理方式。 | 产品与用户运营负责人 |
