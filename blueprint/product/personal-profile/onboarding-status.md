...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P1 | 首次查询已将个人档案建立为 `TBC`，但本次仍返回 `NONE`，返回状态与服务终态不一致；应由产品确认首次查询应返回 `NONE` 还是 `TBC`。 | 产品与用户运营负责人 |
| P1 | 查询动作当前会在无档案时直接建立个人档案；查询是否应承担建档职责，还是只报告状态，应由产品确认。 | 产品与用户运营负责人 |
| P2 | `user_tennis_profile.status` 的建表定义和注释使用小写值，实际读写使用大写枚举值且按大小写精确识别；应由产品与数据负责人统一存量数据和取值口径。 | 产品与用户运营负责人 |
| P1 | 同一用户首次重复发起查询时，重复建档冲突没有专门的业务提示，也不会改为返回已存在档案的状态；应由产品确定对外结果。 | 产品与用户运营负责人 |
