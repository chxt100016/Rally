...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P2 | 数字型 `category` 低于 `250` 的赛事会被排除，空值和非数字值反而会保留；该门槛、单位及不同格式的业务含义需职业赛事数据负责人确认。 | 产品与职业赛事数据负责人 |
| P1 | `ONGOING` 与 `UPCOMING` 会被映射为同一存储状态 `active`，且未另行按日期限定；两个查询条件是否应分别交付即将开始和进行中赛事，需产品与数据负责人确认。 | 产品与职业赛事数据负责人 |
| P1 | 存储状态用 `active` / `completed`，交付状态却完全由日期推导；两者冲突时哪个代表业务事实，需职业赛事数据负责人确认。 | 产品与职业赛事数据负责人 |
| P2 | `status` 只识别全大写值，`range` 却不区分大小写；查询条件的大小写口径需产品负责人确认。 | 产品与职业赛事数据负责人 |
| P1 | `recent` 固定取今日前后各一个月，`live` 只判断赛期是否覆盖今日且强制存储状态为 `active`；命名、时间窗口与覆盖口径的业务意图需产品与数据负责人确认。 | 产品与职业赛事数据负责人 |
| P2 | 同期分组只比较城市和赛期，不比较赛事名称；同城同期的不同赛事是否应归为一组，需内容与数据负责人确认。 | 产品与职业赛事数据负责人 |
| P2 | 同期分组只将新赛事与组内首项比较，没有用赛期的传递交集合并整组；重叠链赛事的分组标准需内容负责人确认。 | 产品与职业赛事数据负责人 |
| P2 | `groupId` 仅按本次查询结果顺序临时生成，条件或数据变化后同一赛事可能获得不同值；它是否可被下游作为稳定标识，需产品负责人确认。 | 产品与职业赛事数据负责人 |
| P1 | `tour` 建表说明包含 `ATP`、`WTA`、`ITF`、`Grand`，但职业赛事采集业务只定义 `ATP` 与 `WTA`；查询时又不校验 `type`，有效类型范围需职业赛事数据负责人确认。 | 产品与职业赛事数据负责人 |
| P1 | 主信息以 `tournament_id` 与 `year` 共同唯一，查询项却不交付 `year` 且只用 `tournament_id` 作为 `id`；同一外部赛事标识跨年复用时的唯一识别方式需产品与数据负责人确认。 | 产品与职业赛事数据负责人 |
| P1 | 背景图七牛云访问地址有效期固定为 3600 秒，该时长与过期后的对外口径需内容与平台负责人确认。 | 产品与职业赛事数据负责人 |
| P2 | 查询不分页且没有数量上限；全量结果的交付规模及排序保证需产品与数据负责人确认。 | 产品与职业赛事数据负责人 |
