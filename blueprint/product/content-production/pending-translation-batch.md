...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P2 | 每批最多 `50` 条为写死规则，没有业务依据或配置入口；批量上限与调整责任人需由内容及翻译负责人确定。 | 产品与内容运营负责人 |
| P2 | 本服务一次处理全部业务对象类型和目标语言，没有筛选、优先级、数量上限或稳定顺序；大批积压时的处理次序和单次范围待内容负责人确定。 | 产品与内容运营负责人 |
| P2 | DeepSeek 使用的模型写死为 `deepseek-v4-pro`，且没有备用翻译来源；模型选型、不可用时的降级方式及维护责任人待定。 | 产品与内容运营负责人 |
| P2 | DeepSeek 必须按输入顺序逐行返回且数量完全一致，结果没有携带翻译条目标识；错序但行数一致时无法识别，对应关系与验收机制待翻译负责人确定。 | 产品与内容运营负责人 |
| P1 | 外部整批失败或返回数量不一致时只跳过该批，没有重试，也不向内容人员说明失败条目和原因；失败反馈与重处理方式待定。 | 产品与内容运营负责人 |
| P1 | 单条空白译文会保持待译，但没有失败原因、尝试次数或最后尝试时间；长期无法翻译条目的治理方式待定。 | 产品与内容运营负责人 |
| P1 | 非空译文只做首尾空白清理，不校验语言、专有名词、长度或内容质量；自动保存前是否需要审核及质量标准待翻译负责人确定。 | 产品与内容运营负责人 |
| P2 | 现有查询只把 `translated_text` 为空字符串视为待译，但领域注释把 `NULL` 也描述为待译；历史 `NULL` 数据是否存在、应否纳入处理需由数据负责人确认。 | 产品与内容运营负责人 |
| P2 | `translation` 表注释中的 `entity_type`、`language` 示例与实际枚举口径不一致；数据库说明应以何种值为准需由数据负责人统一。 | 产品与内容运营负责人 |
| P1 | 翻译条目以业务对象类型、原文前 200 个字符和目标语言保证唯一，超过 200 字符但前缀相同的原文可能冲突；唯一性口径待数据负责人确定。 | 产品与内容运营负责人 |
| P1 | 入口未体现内容人员身份或权限校验；谁可以触发全量翻译及费用控制规则待产品与安全负责人确定。 | 产品与内容运营负责人 |
