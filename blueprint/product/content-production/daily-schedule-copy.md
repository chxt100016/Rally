...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P2 | 文案日期固定取服务运行当天且固定输出简体中文，是否允许指定日期与语言由内容业务负责人确定。 | 产品与内容运营负责人 |
| P2 | 当前赛事窗口写死为开始日期不晚于明日、结束日期不早于昨日；该缓冲口径及跨时区日期归属由内容业务负责人确定。 | 产品与内容运营负责人 |
| P1 | 数字赛事级别低于 `250` 时被排除，空值和非数字级别则保留；该展示门槛和异常值口径缺少业务说明，由职业赛事数据与内容负责人确定。 | 产品与内容运营负责人 |
| P2 | 赛事固定按 `GS`、`1000`、`500`、`250`、其他级别排序，再按开始日期、结束日期和赛事标识排序；优先级及同级规则由内容业务负责人确定。 | 产品与内容运营负责人 |
| P2 | 同城且日期范围重叠的赛事会被传递性合并为赛事组；城市缺失或日期缺失不合并，是否符合内容编排意图待内容业务负责人确认。 | 产品与内容运营负责人 |
| P2 | 未结束比赛全部纳入，并只补入“至少有一场未结束比赛”的日期上的已结束比赛；每日文案是否应限制为自然日、展示几天及是否展示完赛场次待内容业务负责人确定。 | 产品与内容运营负责人 |
| P2 | 场地分组的排序规则写死：带 `Court + 数字` 的场地按数字升序但排在非数字场地之后，非数字场地按组内最小种子号排序；其业务意图待内容业务负责人确认。 | 产品与内容运营负责人 |
| P2 | 标题连接符、巡回赛摘要顺序、日期格式、段落模板、“vs”“待定”等文案均写死；没有配置入口，也未限制最多赛事、场地或比赛数量，由内容业务负责人确定。 | 产品与内容运营负责人 |
| P2 | 日期段只取排序后第一个非空场地的首场比赛轮次作为整段轮次，混合轮次时可能失真，展示口径待内容业务负责人确定。 | 产品与内容运营负责人 |
| P1 | 进行中和已结束比赛会先以排期位置展示一次状态，并在比分后再次追加相同状态；重复状态是否保留由内容业务负责人确定。 | 产品与内容运营负责人 |
| P2 | 盘分当前只展示 `p1Games-p2Games`，抢七分、盘序和胜方不直接展示；比分呈现规则由内容业务负责人确定。 | 产品与内容运营负责人 |
| P1 | `tour_match.status` 的空值或未识别值会按未结束比赛参与查询，但不会生成状态文案；异常状态的展示与治理责任待职业赛事数据负责人确定。 | 产品与内容运营负责人 |
| P1 | 缺译条目登记失败不会阻止文案交付，也没有对内容人员的提示；是否需要暴露缺译或登记失败信息由翻译与内容业务负责人确定。 | 产品与内容运营负责人 |
