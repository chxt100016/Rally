...
- 批量规则接受小写 `full`，但正式约球状态枚举没有该取值；其业务含义、历史来源和是否继续兼容需产品与数据负责人确认。
- 正式状态包含 `ONGOING`，但本服务只结算 `OPEN` 和 `full`；持久化为 `ONGOING` 且已过结束时间的约球是否也应结算，需产品负责人确认。
- 普通约球和赛事约球使用同一结束规则，代码没有说明赛事约球结束是否还应同步赛事比赛；需赛事产品负责人确认。
- 当前结算每天执行一次，约球实际结束到状态落库之间可能存在延迟；允许的最大延迟需产品负责人确认。
