...
|---|---|---|---|
| 未登录 | 拒绝操作 | 未登录，请先登录 | 报名保持原状态。 |
| 登录凭证失效或无效 | 拒绝操作 | 登录已过期，请重新登录，或登录凭证无效，请重新登录 | 报名保持原状态。 |
| `meetupId` 为空或只有空白 | 拒绝操作 | 活动id不能为空 | 报名保持原状态。 |
| `registrationId` 为空或只有空白 | 拒绝操作 | 报名ID不能为空 | 报名保持原状态。 |
| 目标约球不存在 | 拒绝操作 | 约球不存在 | 报名保持原状态。 |
| 指定报名不存在，或不属于目标约球 | 拒绝操作 | 报名记录不存在 | 该约球下全部报名保持原状态。 |
| 当前用户不是约球发布者 | 拒绝操作 | 仅发布者可操作 | 报名保持原状态。 |
| 报名已经不是 `PENDING` | 拒绝操作 | 该报名当前状态不可撤回 | 报名保持原状态。 |
| 报名状态保存失败 | 拒绝失败 | 系统异常，请稍后重试 | 报名状态不变。 |
