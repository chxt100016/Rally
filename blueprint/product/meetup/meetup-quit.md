...
| 发布者没有微信小程序接收身份或未订阅 | 触达日志记 `SKIPPED` | 退出成功 | 退出结果保留。 |
| 通知触发或微信渠道发生异常 | 触达日志记 `FAILED`；无法建日志时只记应用错误 | 退出成功 | 退出结果保留；发布者可能收不到通知。 |
...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P2 | 临近开始退出会按 `meetup.quit.penalty_threshold_hours` 判定为应处罚，但实际信誉分扣减尚未实现，`meetup.quit.penalty_under_6h` 也未被读取；处罚是否启用、扣多少、落在哪个业务对象及何时生效需由产品与风控负责人确定。 | 产品与约球运营负责人 |
| P1 | 当前没有按约球状态或开始、结束时间限制退出，`ONGOING`、`CLOSED`、`FINISHED` 下仍可退出；活动生命周期内各状态是否允许退出需由产品负责人确定。 | 产品与约球运营负责人 |
| P2 | `REVIEWED` 和 `SKIPPED` 报名在赛后仍可改为 `QUIT`，从而不再计入有效人数；赛后参与事实是否应被退出覆盖需由产品负责人确定。 | 产品与约球运营负责人 |
| P2 | 约球发布者也有一条有效参与关系，当前没有禁止发布者从本服务退出；退出后 `creator_id` 仍指向本人、人数可能降为零，且本人会成为成员退出通知的接收人，发布者应关闭约球还是允许退出需由产品负责人确定。 | 产品与约球运营负责人 |
| P1 | 退出将报名改为 `QUIT`，但不更新 `opt_time`；该字段是否应统一记录最近一次状态操作时间需由产品与数据负责人确定。 | 产品与约球运营负责人 |
| P1 | 多名参与者同时退出时，报名终态与 `current_players` 缺少明确的冲突判定和一致性规则，需由产品与数据负责人确定。 | 产品与约球运营负责人 |
| P2 | 成员退出场景的定义写有“创建人/全体已加入参与人”，当前只通知约球发布者；最终接收人范围需由产品负责人确定。 | 产品与约球运营负责人 |
