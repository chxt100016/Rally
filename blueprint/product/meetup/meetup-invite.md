...
| 收件人没有可用的 `TEAM_SUCCESS` 微信订阅消息授权 | 跳过该收件人的提醒 | 邀请成功 | 报名和群聊成员关系保持成功，授权状态不变。 |
| 通知发送前收件人已退出约球 | 跳过该收件人的提醒 | 邀请成功 | 报名和群聊成员关系保持成功，该用户授权不消费。 |
| 收件人没有微信小程序接收身份，或微信发送失败 | 记录通知失败，不影响邀请 | 邀请成功 | 报名和群聊成员关系保持成功，已占用授权记为 `FAILED`。 |
...
- 本服务不为邀请单独取得或登记微信订阅消息授权；只有邀请后组团成功时，才尝试消费参与者已有授权发送提醒。
- 微信订阅消息是否实际送达不改变 `JOINED` 终态；邀请结果不等待微信通知结果。
- 本服务只向发布者返回成功标识，不交付更新后的报名、约球人数、群聊资料或通知结果。

## 遗留与待定

- 邀请会跳过个人档案、性别、NTRP、信誉分和时间冲突等普通报名准入规则；发布者是否有权豁免、哪些规则仍须保留，需由产品与风控负责人确定。
- 邀请不校验被邀请人的用户或账户是否存在；无效用户编号仍可能形成报名和群聊成员关系，身份有效性规则需由产品与账户负责人确定。
- 邀请不校验约球状态、时间、加入模式或约球类型，因此草稿、进行中、已关闭、已结束及赛事约球也可能接受邀请；允许范围需由产品负责人确定。
- 邀请直接产生 `JOINED`，没有被邀请人接受或拒绝的等待阶段；是否需要邀请确认以及拒绝后的收场规则需由产品负责人确定。
- 新报名没有记录邀请来源或邀请人，后续无法区分发布者邀请与用户直接报名；是否需要保留邀请关系需由产品与数据负责人确定。
- 未满员时不会向被邀请人发送邀请成功通知；满员时也只尝试发送组团成功提醒，邀请结果的告知方式需由产品与通知负责人确定。
- 邀请没有名额占用或并发版本条件；多人同时被邀请时可能超过 `max_players`，名额分配与超员收场规则需由产品负责人确定。
- `rally_meetup_registration` 只按 `biz_id` 唯一，没有限制同一用户在同一约球下只能有一条未终止报名；并发邀请可能产生重复报名，数据唯一性规则需由产品与数据负责人确定。
- 报名已是 `JOINED` 但群聊成员缺失时会被视为重复报名，群聊关系无法通过本服务补齐；报名与群聊不一致的修复规则需由产品负责人确定。
- 群聊成员已存在但没有未终止报名时，邀请会失败且不补齐报名；历史群聊关系的清理或恢复规则需由产品负责人确定。
- 只要邀请后达到人数上限就尝试发送组团成功提醒，没有记录组团成功是否已经通知；重复提醒的判定需由产品与通知负责人确定。
- 微信订阅消息授权或发送失败均不向发布者反馈；是否需要展示通知未生效需由产品负责人确定。
