...
5. 事务提交后，为仍符合接收条件的候选人按事件、接收人和渠道建立唯一触达日志，并直接尝试发送约球名称、时间、地点和“创建人取消”。
...
| 参与者在通知发送前已退出 | 跳过该参与者的通知 | 关闭成功 | 约球保持 `CLOSED`；不建立该用户的触达日志。 |
| 参与者没有微信小程序接收身份或未订阅 | 触达日志记 `SKIPPED` | 关闭成功 | 约球保持 `CLOSED`。 |
| 微信渠道发送失败 | 触达日志记 `FAILED` | 关闭成功 | 不回滚关闭。 |
...
- 关闭结果不等待渠道触达；未订阅、缺少接收身份或发送失败不改变约球的 `CLOSED` 终态，也不保证参与者一定收到消息。
