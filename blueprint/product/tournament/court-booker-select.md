...
- 当前只校验认领者是该场比赛参与者，不校验赛事报名的 `court_ability`；不能订场的参赛者是否也可认领，需赛事产品负责人确认。
- 比赛不存在时对外提示“报名记录不存在”，没有区分比赛与报名；是否需要改为比赛不存在，需赛事产品负责人确认。
- 任一非 `MATCHED` 状态都提示“订场人已被选定”，其中已终止或已完成比赛并不等同于刚完成选定；各状态是否需要区分提示，需赛事产品负责人确认。
- 数据库建表说明中的比赛状态清单未列出代码实际使用的 `PENDING_PLAY`；数据库定义与业务状态清单应以哪一方为准，需赛事产品负责人和数据负责人共同确认。
- 当前认领成功后不通知同场其他参赛者；是否需要通知及通知渠道，需赛事运营负责人确认。
