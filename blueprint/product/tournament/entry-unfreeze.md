...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P1 | 当前只校验赛事为 `ACTIVE` 且未晚于 `end_time`，不检查报名窗口、资格赛截止时间、当前轮次或报名阶段；这些条件是否应限制解冻，需赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P2 | `rally_tournament.end_time` 允许为空，且现有赛事创建、编辑与比赛推进流程均未看到明确写入该字段；赛事何时结束、由谁写入结束时间，需赛事产品与数据负责人确定。 | 产品与赛事运营负责人 |
| P2 | 当前时间恰好等于 `end_time` 时仍允许解冻，只有晚于该时刻才拒绝；结束时刻是否应包含在可解冻时间内，需赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P1 | 手机号条件只判断 `user.phone` 是否非空，不在解冻时重新验证号码归属、有效性或微信授权状态；解冻所需的手机号身份强度需账户与赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P1 | 本服务不检查冻结报名是否已有未结束的赛事比赛；出现冻结状态与比赛状态并存时能否直接回到匹配池，需赛事产品负责人确定。 | 产品与赛事运营负责人 |
