...
8. 向同场除拒绝人以外的参与者发起 `TOURNAMENT_REJECTED` 微信订阅消息，消息包含赛事名称和固定的拒绝提示。
9. 向拒绝人返回处理成功；微信触达结果不改变比赛终止和报名回池的结果。
...
| 通知接收人没有微信小程序身份或未订阅 | 触达日志记 `SKIPPED` | 拒绝成功 | 不撤销比赛终止、报名回池或赛约关闭。 |
| 微信渠道发送失败 | 触达日志记 `FAILED` | 拒绝成功 | 不撤销比赛终止、报名回池或赛约关闭。 |
...
- 本服务不保证微信订阅消息送达；通知按比赛事件、接收人和渠道去重，触达失败不改变拒绝终态。
...
| 等级 | 待定的是什么 | 要谁来定 |
|---|---|---|
| P1 | 当前不要求拒绝人的 `result_confirm_status` 必须为 `PENDING`；赛果提交人或已确认的参赛者仍可拒绝赛果，若异常数据中比赛仍为 `PENDING_CONFIRM` 而本人已为 `REJECTED`，也会再次进入拒绝逻辑。允许范围与幂等口径需赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P1 | 当前只要拒绝人存在赛事报名即可继续，不校验报名的 `status`、`current_round` 与比赛的关系，并以报名 `stage` 选择拒绝上限和累计字段。可拒绝资格与阶段一致性需赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P1 | 双打队友共享 `entry_no`，但拒绝次数按拒绝人各自的报名累计；拒绝额度应按个人还是按参赛单元计算，需赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P2 | 赛果拒绝保留 `winner_entry_no`、`submitted_by` 和 `submitted_time`，且比赛进入 `REJECTED` 后没有重报赛果路径；这些已拒绝数据的展示、追溯与后续处置规则需赛事运营负责人确定。 | 产品与赛事运营负责人 |
| P2 | 比赛表已有 `reject_phase`、`rejected_by`、`rejected_time`，但当前流程只写 `reject_reason_code`；赛果拒绝阶段、拒绝人和发生时间的审计口径需赛事运营与数据负责人确定。 | 产品与赛事运营负责人 |
| P2 | 赛果拒绝终止比赛时不写 `completed_time`；是否需要统一的终止时间需赛事运营与数据负责人确定。 | 产品与赛事运营负责人 |
| P1 | 赛果拒绝只把仍为 `IN_MATCH` 的同场报名改为 `WAITING`；其他状态原样保留，终止比赛后可能出现同场报名去向不一致。异常状态的处置规则需赛事产品负责人确定。 | 产品与赛事运营负责人 |
| P1 | 赛约在全员确认后已通常从 `DRAFT` 转为 `OPEN`，而本服务只关闭 `DRAFT`；因此正常赛果被拒绝后，关联赛约仍可能保持 `OPEN` 或更后续状态。比赛终止后赛约的正式终态需赛事与约球产品负责人共同确定。 | 产品与赛事运营负责人 |
| P2 | 拒绝通知固定表达为“对手使用拒绝比赛机会”，不展示本次 `reject_reason_code`，三人资格赛也沿用“对手”单数；通知文案与理由披露范围需赛事运营负责人确定。 | 产品与赛事运营负责人 |
...
| P2 | `rally_meetup.status` 的建表缺省值和说明使用小写，当前赛约业务读写使用大写枚举；正式存储取值与历史数据兼容方式需赛事与约球数据负责人确定。 | 产品与赛事运营负责人 |
