...
- 微信支付查单失败与真实未支付当前都对外返回 `UNPAID`，是否需要向付款人区分“暂时无法核实”由产品与支付业务负责人确定。
- `CLOSED`、`FAILED` 支付单不会再次查询微信支付，发生迟到付款时如何核对和处置由支付业务负责人确定。
- 已是 `PAID` 的支付单不会重新推进关联报名；支付单已付但报名或席位未推进时的修复入口与责任归属待支付及赛事业务负责人确定。
- `pay_time` 当前记录本服务确认付款的时间，不是微信支付实际完成付款的时间；业务统计应采用哪一种时间口径待支付业务负责人确定。
- `FAILED` 状态已经定义，但现有入口中未发现将支付单置为该状态的业务规则，其适用场景由支付业务负责人确定。
